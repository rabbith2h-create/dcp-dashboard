...
export default function App() {
  ...
  const clientesUnicos = Math.round(totalBonos * 0.757);
  const proyeccion = data.length > 0 ? Math.round((totalBonos / data.length) * 31) : 0;
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "#4a7a94", marginTop: 2 }}>
            Dashboard en tiempo real · Firebase Realtime Database
          </div>
        </div>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{
            width: 8, height: 8, borderRadius: "50%", background: "#4ecdc4",
            boxShadow: "0 0 8px #4ecdc4", animation: "pulse 2s infinite"
          }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 28 }}>
        {[
          { label: `Total de bonos entregados ${lastFecha}`, value: formatNum(totalBonos), accent: "#5bc8f5" },
          { label: "Clientes únicos recibiendo megas gratis", value: formatNum(clientesUnicos), accent: "#4ecdc4" },
          { label: "Proyección usuarios únicos (mes)", value: formatNum(proyeccion), accent: "#a78bfa" },
        ].map((kpi, i) => (
          <div key={i} style={{
      ...
      <div style={{
        ...
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={chartData} margin={{ top: 24, right: 10, left: 10, bottom: 0 }}>
            ...
            <YAxis tick={{ fill: "#4a7a94", fontSize: 11 }} axisLine={false} tickLine={false}
              tickFormatter={v => v.toLocaleString()} width={75} />
